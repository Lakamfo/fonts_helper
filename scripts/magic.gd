...
func _ready() -> void:
	...
	if debug or force_debug:
		standart_path = "res://assets/default_fonts/"
		preset_path = "res://preset.txt"
		export_fonts_path = "res://export"
...
func add_font(path: String) -> void:
	...
	example_dyfont.set("font_data",0)
...
func dragNdrop(files=[], _screen = 0) -> void:
	if '.ttf' or '.otf' in files[0]:
		...
		example_dyfont.set("font_data",loaded_fonts[loaded_fonts.size()- loaded_fonts.size()])
...
func add_bt() -> void:
	var instance = panel.instance()
	container.add_child(instance)
	instance.label.text = str(get_tree().get_nodes_in_group("panel").size() + 1)
